                             ...
                             mat4 const& T)
{
	...
	return mesh().intersectImplTriTest(ray_local,p1,p2,p3).t;
    //return -1;
}
...
void RunYourTests()
{
	...
	RunTest(
        "My sphere",
		Test_RaySphereIntersect(glm::vec3(0,1,-4), glm::vec3(0,-SQRT_HALF,-SQRT_HALF), BACK5_MATRIX),
		SQRT_TWO-1);
	RunTest(
        "My spheres",
		Test_RaySphereIntersect(glm::vec3(0,0,0), glm::vec3(SQRT_HALF,0,SQRT_HALF), IDENTITY_MATRIX),
		1.0);
	RunTest(
        "My poly",
		Test_RayPolyIntersect(glm::vec3(0,0,0.5), glm::vec3(-SQRT_HALF,0,-SQRT_HALF), POINT_N1N10, POINT_1N10, POINT_010, IDENTITY_MATRIX),
		SQRT_HALF);

	 RunTest(
        "My cube",
		Test_RayCubeIntersect(glm::vec3(3,0,2), glm::vec3(-SQRT_HALF,0,-SQRT_HALF), DOUBLE_MATRIX),
		2*SQRT_TWO);

	 RunTest(
        "My cylinder",
		Test_RayCylinderIntersect(glm::vec3(0,-2.5,1.5), glm::vec3(0,SQRT_HALF,-SQRT_HALF), IDENTITY_MATRIX),
		2*SQRT_TWO);
}
